            ...
            <form action="" method="POST" id="{{ $form_name }}">
                ...
                <div class="form-group">
                    @foreach($langs as $lang)
                        ...
                        <input type="text" value="{{ $categoria->{'nome_'.$lang} }}" name="nome_{{$lang}}" id="nome_{{$lang}}" class="form-control mb-2" />
                    @endforeach
                </div>
                <div class="form-group">
                    @foreach($langs as $lang)
                        ...
                        <textarea id="desc_{{$lang}}" style="min-height: 100px;" name="desc_{{$lang}}" class="form-control summernote mb-2"  >{{$categoria->{'desc_'.$lang} }}</textarea>
                    @endforeach
                </div>
                <div class="form-group">
...
<script>
    $("#{{$form_name}}").validate({
        ...
        submitHandler: function (form)
        {
            $.ajax({
                type: "PUT",
                url: "{{route('category.update',[$categoria->id])}}",
